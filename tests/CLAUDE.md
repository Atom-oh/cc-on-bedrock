...
## Key Files
- `integration/test-e2e.sh` - 전체 E2E 테스트 (Docker, Terraform, Next.js, ShellCheck)
...
DASHBOARD_URL=https://cconbedrock-dashboard.<domain> \
  COGNITO_ACCESS_TOKEN=<token-from-cc-bedrock-local-client> \
  TEST_USER_EMAIL=<email> \
  TEST_USER_SUBDOMAIN=<subdomain> \
  TEST_USER_DEPT=<dept> \
...
- Local Governance 테스트는 실제 Bedrock 호출과 IAM 변경이 일어남 — 격리된 테스트 사용자 email/subdomain으로 실행할 것
