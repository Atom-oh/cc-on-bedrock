...
describe("buildUserDailyTrend", () => {
  it("pivots per (user, date) summing cost and tokens", () => {
    const recs = [
    ...
    expect(d11.alice).toBeCloseTo(1.5);
    const tok12 = t.tokens.find((p) => p.date === "2026-06-12")!;
    expect(tok12.bob).toBe(200);
  });
  ...
  it("generates the full calendar axis with zero-fill (no gaps)", () => {
    ...
    expect(t.cost.find((p) => p.date === "2026-06-12")!.alice).toBe(0);  // gap day → 0
  });
  ...
  it("keeps Top-N by cost and folds the rest into one 'others' series", () => {
    ...
    expect(keys).toEqual(["u1", "u2", "others"]);
    expect(t.othersCount).toBe(2);
    expect(t.cost[0].others).toBeCloseTo(15); // u3 + u4
  });
  ...
  it("emits no 'others' when users <= topN", () => {
    ...
    expect(t.series.map((s) => s.key)).toEqual(["u1"]);
