...
export default function ArchitecturePage() {
  ...
  return (
    <PageShell
      ...
      subtitle={t(
        "가용성, 보안, 사용자 격리에 중점을 둔 8 스택 구성. 24 개의 ADR이 주요 결정의 근거를 문서화합니다.",
        "8-stack composition focused on availability, security, and per-user isolation. 24 ADRs document the rationale of each major decision."
      )}
      tags={[
        { label: "ADR-004", color: "purple" },
        { label: "ADR-013", color: "purple" },
        { label: "ADR-014", color: "purple" },
        { label: "ADR-022", color: "purple" },
      ]}
    >
      ...
      <Table
        ...
        rows={[
          { n: "01", stack: "Network", what: "VPC (10.100.0.0/16) · NAT · VPC Endpoints · DNS Firewall · Route 53", adr: "—" },
          { n: "02", stack: "Security", what: "Cognito + custom attrs · ACM · KMS · Secrets Manager · IAM + Permission Boundary", adr: "—" },
          { n: "03", stack: "Usage Tracking", what: t("usage / cli_tokens / user_budgets / approval_requests / prompt_audit / mcp_catalog 테이블, tracker / budget-check / ec2-idle-stop / audit / gateway-manager Lambda", "usage / cli_tokens / user_budgets / approval_requests / prompt_audit / mcp_catalog tables, tracker / budget-check / ec2-idle-stop / audit / gateway-manager Lambdas"), adr: "011, 019" },
          { n: "04", stack: "ECS Dashboard infra", what: t("NLB internet-facing · Nginx Fargate (HA) · DynamoDB routing table + Lambda nginx-config-gen", "NLB internet-facing · Nginx Fargate (HA) · DynamoDB routing table + Lambda nginx-config-gen"), adr: "002" },
          { n: "05", stack: "Dashboard", what: t("Next.js standalone ECS · 통합 CloudFront · Lambda@Edge (session-validator + origin-router)", "Next.js standalone ECS · Unified CloudFront · Lambda@Edge (session-validator + origin-router)"), adr: "013, 016, 017" },
          { n: "06", stack: "WAF", what: t("CLOUDFRONT-scope WebACL (us-east-1)", "CLOUDFRONT-scope WebACL (us-east-1)"), adr: "—" },
          { n: "07", stack: "EC2 DevEnv", what: t("Launch Template (ARM64 t4g.large) · DLP SG 3-tier (open/restricted/locked) · Hibernation", "Launch Template (ARM64 t4g.large) · DLP SG 3-tier · Hibernation"), adr: "004, 005, 009, 010, 018" },
          { n: "08", stack: "Local Governance", what: t("STS Issuer + Function URL · cc-on-bedrock-limits · token-limit-enforcer · UserRoleProvisioner + EventBridge `cognito-user-created` + DLQ", "STS Issuer + Function URL · cc-on-bedrock-limits · token-limit-enforcer · UserRoleProvisioner + EventBridge cognito-user-created + DLQ"), adr: "014, 015, 021, 022, 024" },
        ]}
      />
      ...
      <H2 id="ec2">{t("2. EC2-per-user DevEnv (ADR-004)", "2. EC2-per-user DevEnv (ADR-004)")}</H2>
      <P>
        {t(
          "각 사용자에게 전용 EC2 인스턴스 1대가 할당됩니다. ECS Task 모델은 ADR-004로 폐기됨.",
          "Each user is allocated a single dedicated EC2 instance. The ECS Task model was deprecated by ADR-004."
        )}
      </P>
      <ul className="text-sm text-gray-400 space-y-1.5 list-disc pl-5 mb-5">
        <li>{t("1 EC2 instance — t4g.large ARM64 (Ubuntu 24.04 또는 AL2023, ADR-018)", "1 EC2 instance — t4g.large ARM64 (Ubuntu 24.04 or AL2023, ADR-018)")}</li>
        ...
        <li>{t("3 DevEnv 포트 — code-server :8080, Frontend :3000, API :8000 (ADR-009)", "3 ports — code-server :8080, Frontend :3000, API :8000 (ADR-009)")}</li>
        <li>{t("Hibernation 지원 (ADR-010) — RAM → 암호화 EBS, ~5초 resume", "Hibernation (ADR-010) — RAM → encrypted EBS, ~5s resume")}</li>
      </ul>

      <H2 id="local">{t("3. Local Governance Mode (ADR-014)", "3. Local Governance Mode (ADR-014)")}</H2>
      <P>
        {t(
  ...
  class STS lambda`}
      ...
      <Table
        ...
        rows={[
          ...
          },
          {
            ...
            perf: t("10~20s · 8+ tools (per-dept MCP, ADR-007)", "10–20s · 8+ tools (per-dept MCP, ADR-007)"),
          },
        ]}
      />
      <P>
      ...
      <H2 id="sso">{t("5. 인증 / SSO (ADR-013)", "5. Authentication / SSO (ADR-013)")}</H2>
      <P>
        {t(
      ...
      <Callout type="tip" title={t("자세히는 ADR을", "Dig into the ADRs")}>
        <a href="https://github.com/Atom-oh/cc-on-bedrock/tree/main/docs/decisions" target="_blank" rel="noreferrer" className="text-accent-cyan hover:underline">
          docs/decisions/ — {t("24 개 ADR 전체 목록", "Full list of all 24 ADRs")}
        </a>
      </Callout>
    </PageShell>
  );
