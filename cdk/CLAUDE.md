...
- `lib/lambda/nginx-config-gen.py` - Nginx 설정 생성 (DynamoDB Stream → S3). code-server(8080) built-in + 사용자 customRoutes(seed: root→3000, /api→8000) 검증·렌더 (ADR-026). 보간 전 subdomain·container_ip·path·port 전수 재검증(B-H1), 세그먼트 경계 location
