...
**Extended by:** [ADR-027 Cognito trigger fallback provisioning](ADR-027-cognito-trigger-fallback-provisioning.md) — federated JIT users emit no `AdminCreateUser`/`SignUp` CloudTrail event, so the EventBridge path never fires for them
