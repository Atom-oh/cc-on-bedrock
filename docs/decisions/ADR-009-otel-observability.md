...
- **Privacy / DLP (no-content).** `OTEL_LOG_TOOL_DETAILS=1` would log bash commands / file paths / prompts; the **collector logs pipeline scrubs** them — keeps only records carrying `tool_name` (the filter above — in practice `tool_result`/`tool_decision`, since only those emit it), lifts `skill_name`/`subagent_type` to top-level, deletes `tool_parameters`/`tool_input`/`prompt`/`response` before S3. No prompt/text/image content is ever persisted.
