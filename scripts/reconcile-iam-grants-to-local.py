...
Local role (cc-on-bedrock-local-user-{sub}). This script finds inline grant
...
def gaps(task_grants: dict, local_grants: dict, sub_by_subdomain: dict) -> list:
    ...
    local_grants:       {sub: set(policyName)}          grant policies on each local role
    sub_by_subdomain:   {subdomain: sub}                identity mapping (Cognito)
    Returns list of (local_role_name, policy_name, subdomain) needing copy.
    """
    out = []
    for subdomain, names in sorted(task_grants.items()):
        sub = sub_by_subdomain.get(subdomain)
        if not sub:
            continue  # no Cognito identity → skip (orphan/stale task role)
        have = local_grants.get(sub, set())
        for name in sorted(names):
            if is_grant_policy(name) and name not in have:
                out.append((f"cc-on-bedrock-local-user-{sub}", name, subdomain))
    return out


def _self_test() -> int:
    task = {"alice": {"Grant-r1", "PolicySet-s3", "BedrockInvokeInline"}, "bob": {"Grant-r2"}}
    local = {"sub-a": {"Grant-r1"}, "sub-b": set()}
    sub_by = {"alice": "sub-a", "bob": "sub-b"}
    res = gaps(task, local, sub_by)
    ...
    assert got == {("cc-on-bedrock-local-user-sub-a", "PolicySet-s3"), ("cc-on-bedrock-local-user-sub-b", "Grant-r2")}, got
    # unmapped subdomain is skipped
    assert gaps({"ghost": {"Grant-x"}}, {}, {}) == []
...
def main() -> int:
    ...
    for page in cognito.get_paginator("list_users").paginate(UserPoolId=pool_id):
        for u in page.get("Users", []):
            ...
            sd, sub = attrs.get("custom:subdomain"), attrs.get("sub")
            if sd and sub:
                sub_by_subdomain[sd] = sub
    ...
    todo = gaps(task_grants, local_grants, sub_by_subdomain)
    if not todo:
