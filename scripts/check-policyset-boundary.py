...
def _self_test() -> int:
    ...
    print("[self-test] OK — boundary⊇allowlist comparison logic verified")
    return 0
...
def main() -> int:
    ...
    asvcs = allowlist_services(Path(args.allowlist_source))
    missing = check(asvcs, bsvcs)
    print(f"allowlist services: {sorted(asvcs)}")
    print(f"boundary services : {sorted(bsvcs)}")
    if missing:
        print(f"[FAIL] services requestable but NOT in boundary (grants would be silently nullified): {sorted(missing)}", file=sys.stderr)
        return 1
    print("[OK] boundary ⊇ request service allowlist")
    return 0
